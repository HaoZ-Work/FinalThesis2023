...
#             masked_lm_loss = loss_fct(prediction_scores,masked_lm_labels).view(bsize, seqlen)
...
# def test_RobertaForMaskedLMwithLoss():
#     # Instantiate a config object for Roberta model
#     config = RobertaConfig(vocab_size=30522, hidden_size=768, num_attention_heads=12, num_hidden_layers=12)
#
#     # Instantiate the RobertaForMaskedLMwithLoss model
#     model = RobertaForMaskedLMwithLoss(config)
#
#     # Move the model to the GPU if available
#     device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
#     model = model.to(device)
#
#     # Generate some random data to feed into the model
#     input_ids = torch.randint(0, 30522, (2, 50)).to(device)  # A batch of 2 sentences of length 50 each
#     attention_mask = torch.ones((2, 50)).to(device)  # Assume all positions are valid (not masked)
#     masked_lm_labels = input_ids.clone()  # Just for the test, assume that the targets are the same as the inputs
#
#     # Forward pass through the model
#     outputs = model(input_ids=input_ids, attention_mask=attention_mask, masked_lm_labels=masked_lm_labels)
#
#     # Check the output dimensions
#     print('Size of the masked language model loss:', outputs[0].size())
#     print('Size of the prediction scores:', outputs[1].size())
#     print('Size of the sequence output:', outputs[2].size())
...
class T5ForMaskedLMwithLoss(T5ForConditionalGeneration):
    #
    def __init__(self, config):
        super().__init__(config)
    #
    def forward(
        ...
        if masked_lm_labels is not None and decoder_input_ids is None and decoder_inputs_embeds is None:
            # get decoder inputs from shifting lm labels to the right
            decoder_input_ids = self._shift_right(masked_lm_labels)
        ...
        outputs = (lm_logits,sequence_output)

        if masked_lm_labels is not None:
            loss_fct = torch.nn.CrossEntropyLoss(ignore_index=-100,reduction='none')
            # move labels to correct device to enable PP
            masked_lm_labels = masked_lm_labels.to(lm_logits.device)
            bsize, seqlen = input_ids.size()

            lm_logits=lm_logits.view(-1, self.config.vocab_size)

            masked_lm_labels=masked_lm_labels.view(-1)
            #lm_logits [bsize*seqlen, vocab_size]
            #masked_lm_labels [bsize*seqlen]
            masked_lm_loss = loss_fct(lm_logits,masked_lm_labels).view(bsize, seqlen)
...
def test_T5ForMaskedLMwithLoss():
    ...
    input_ids = torch.randint(0, 30522, (2, 50)).to(device)  # A batch of 2 sentences of length 50 each
    attention_mask = torch.ones((2, 50)).to(device)  # Assume all positions are valid (not masked)
    lm_labels = input_ids.clone()  # Just for the test, assume that the targets are the same as the inputs

    # Forward pass through the model
    outputs = model(input_ids=input_ids, attention_mask=attention_mask, masked_lm_labels=lm_labels)
...
def main():
    #test_RobertaForMaskedLMwithLoss()
    # test_T5ForMaskedLMwithLoss()
    pass
