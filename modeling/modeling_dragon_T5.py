...
class TextKGMessagePassing(ModelClass):
    ...
    def forward(self, input_ids, token_type_ids, attention_mask, special_tokens_mask, H, A, node_type, node_score, special_nodes_mask, cache_output=False, position_ids=None, head_mask=None, output_hidden_states=True):
        ...
        #print("extended_attention_mask", extended_attention_mask.size()) extended_attention_mask torch.Size([20, 1, 1, 100])



        embedding_output = self.encoder.encoder.embed_tokens(input_ids)
        # print("embedding_output", embedding_output.size()) ([20, 100, 512]) [bs, seq_len, dim]
        ...
        pooled_output = self.pooler(sequence_output,attention_mask.to(torch.bool))
